...
def kernel(tupl):
    ...
    avg_xy_speed = 0
    max_time_diff = 0
    avg_time_diff = (track_data.iloc[track_data.shape[0] - 1]['time'] - track_data.iloc[0]['time']) / track_data.shape[
        0]
    for i in range(1, track_data.shape[0]):
        ...
        if i != track_data.shape[0] - 1:
            angle = calc_angle(track_data.iloc[i - 1]['latitude'], track_data.iloc[i - 1]['longitude'],
                               track_data.iloc[i]['latitude'],
                               track_data.iloc[i]['longitude'], track_data.iloc[i + 1]['latitude'],
                               track_data.iloc[i + 1]['longitude'])
            if angle < min_angle:
                min_angle = angle
                p_index = i
...
def file_to_features(binary_file):
    data = pd.read_csv(BytesIO(base64.b64decode(binary_file)), sep=' ', header=None,
